...
contains
   ...
   subroutine init_response_matrix
      ...
      use linear_solve, only: matrix_inverse
      ...
      use run_parameters, only: lu_option_none, lu_option_local
...
#ifdef ISO_C_BINDING
      if (.not. allocated(phiext_arr)) allocate (phiext_arr(naky, ntubes))
...
!   degradation on some clusters
      ...
      do iky = 1, naky
...
#ifdef ISO_C_BINDING
         do it = 1, ntubes
            if (.not. associated(phiext_arr(iky, it)%eigen)) then
               allocate (phiext_arr(iky, it)%eigen(neigen(iky)))
            end if
         end do
#endif
         ...
         do ie = 1, neigen(iky)
            ...
            do it = 1, ntubes
               if (.not. associated(phiext_arr(iky, it)%eigen(ie)%phiext)) then
                  cptr = transfer(cur_pos, cptr)
                  call c_f_pointer(cptr, phiext_arr(iky, it)%eigen(ie)%phiext, (/nresponse/))
                  cur_pos = cur_pos + nresponse * 2 * nbytes_real
               end if
            end do
#else
...
#ifdef ISO_C_BINDING
            call parallel_matrix_inverse_local(iky)
#else
            call mp_abort('Stella must be built with HAS_ISO_BINDING in order to use local parallel matrix inverse.')
#endif
         case default
            do ie = 1, neigen(iky)
#ifdef ISO_C_BINDING
               if (sgproc0) then
                  ...
                  ! get the matrix inverse so we are ready to solve the linear system
                  call matrix_inverse(response_matrix(iky)%eigen(ie)%zloc)
...
!----------------------------------------------------------!
   ...
   subroutine parallel_matrix_inverse_local(iky)
      ...
      do ijob = 0, njobs - 1
         ...
         do ie = eig_limits(inode, ijob), eig_limits(inode + 1, ijob) - 1
            ...
            call matrix_inverse_local(mp_comm, jroot, win, lu)
   ...
   end subroutine parallel_matrix_inverse_local
