...
!   jobs root process
   ...
   subroutine get_dgdphi_matrix_column(iky, ikx, iz, ie, idx, nz_ext, nresponse, phiext, gext)
      ...
      real, dimension(:), allocatable :: dbdzed_avg

      ia = 1
      if (.not. allocated(dbdzed_avg)) allocate (dbdzed_avg(-nzgrid:nzgrid))

      dbdzed_avg = sum(dbdzed, 1) / size(dbdzed, 1)
      ...
      if (.not. maxwellian_inside_zed_derivative) then
         ...
         do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
            ...
            fac = -0.125 * (1.+time_upwind) * code_dt * vpa(iv) * spec(is)%stm_psi0 &
                  * gyro_fac * spec(is)%zt / delzed(0) * maxwell_vpa_s
...
!                         * (maxwell_mu(ia, iz - 1, imu, is) + maxwell_mu(ia, iz, imu, is))
         ...
         !! GA -- no maxwellian inside derivatives for DKI
         if (.not. driftkinetic_implicit) then
            ...
            do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
               ...
               fac = -0.25 * (1.+time_upwind) * code_dt * vpa(iv) * spec(is)%stm_psi0 &
                     * gyro_fac * spec(is)%zt * maxwell_vpa_s * maxwell_zed(ia, iz)
               !maxwell_vpa(iv, is) * maxwell_mu(ia, iz, imu, is) * maxwell_fac(is)

               !! GA
               mu_dbdzed_p = 1./delzed(0) + mu(imu) * dbdzed_avg(iz) * (1.+zed_upwind)
               mu_dbdzed_m = 1./delzed(0) + mu(imu) * dbdzed_avg(iz) * (1.-zed_upwind)
               ...
               if (stream_sign(iv) < 0) then
                  if (iz > -nzgrid) then
                     ...
                     ! of the homogeneous GKE at this zed index
                     fac0 = fac * ((1.+zed_upwind) * gradpar_fac(iz) &
                                   + (1.-zed_upwind) * gradpar_fac(iz - 1)) * mu_dbdzed_p
                     ! fac1 is the factor multiplying delphi on the RHS
                     ! of the homogeneous GKE at the zed index to the right of
                     ! this one
                     if (iz < nzgrid) then
                     ...
                     fac1 = fac * ((1.+zed_upwind) * gradpar_fac(izp) &
                                   + (1.-zed_upwind) * gradpar_fac(iz)) * mu_dbdzed_m
                  else
                     ! fac0 is the factor multiplying delphi on the RHS
                     ! of the homogeneous GKE at this zed index
                     fac0 = fac * ((1.+zed_upwind) * gradpar_fac(iz) &
                                   + (1.-zed_upwind) * gradpar_fac(nzgrid - 1)) * mu_dbdzed_p
                     ! fac1 is the factor multiplying delphi on the RHS
                     ! of the homogeneous GKE at the zed index to the right of
                     ! this one
                     fac1 = fac * ((1.+zed_upwind) * gradpar_fac(iz + 1) &
                  ...
                  if (iz < nzgrid) then
                     ...
                     fac1 = fac * ((1.+zed_upwind) * gradpar_fac(izm) &
                                   + (1.-zed_upwind) * gradpar_fac(iz)) * mu_dbdzed_m
                  else
                     ! fac0 is the factor multiplying delphi on the RHS
                     ! of the homogeneous GKE at this zed index
...
!      use gyro_averages, only: j0Bmaxwell_avg_ext
