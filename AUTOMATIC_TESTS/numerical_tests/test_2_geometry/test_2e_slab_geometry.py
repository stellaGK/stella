...
def test_slab_metric_analytic_values():
    ...
    # Read per-z data (new format has 12 columns, old has 15; use numpy)
    ...
    # Identify column layout (12=new format, 15=old format)
    ncols = data.shape[1]
    if ncols == 12:
        # New format: grady_dot_grady, gradx_dot_grady, gradx_dot_gradx, B_times_gradB_dot_grady, B_times_kappa_dot_grady
        ...
        # Old format: gds2, gds21, gds22, gds23, gds24, gbdrift, cvdrift, gbdrift0
        gds2    = data[:, 5]
        gds21   = data[:, 6]
        gbdrift = data[:, 10]
        cvdrift = data[:, 11]
    ...
    assert np.allclose(bmag_col, 1.0), \
        f"bmag should be 1.0 everywhere, got {bmag_col}"

    # 2. b.grad z = 1/(q*R)
    expected_bdot = 1.0 / (qinp * rmaj)
    assert np.allclose(bdot_col, expected_bdot, rtol=1e-4), \
        f"b_dot_gradz should be {expected_bdot:.6f}, got {bdot_col}"

    # 3. |∇y|² = 1 + (shat*z)²
    expected_gds2 = 1.0 + (shat * zed_col)**2
    assert np.allclose(gds2, expected_gds2, rtol=1e-4), \
        f"gds2 (|∇y|²) mismatch:\n  expected={expected_gds2}\n  got={gds2}"

    # 4. gds21 = ∇x.∇y * shat = -shat² * z
    expected_gds21 = (-shat * zed_col) * shat
    assert np.allclose(gds21, expected_gds21, atol=1e-4), \
        f"gds21 mismatch:\n  expected={expected_gds21}\n  got={gds21}"

    # 5. Drift terms all zero (flat slab, betaprim=0)
    assert np.allclose(gbdrift, 0.0, atol=1e-10), \
        f"gbdrift should be 0 for flat slab, got {gbdrift}"
    assert np.allclose(cvdrift, 0.0, atol=1e-10), \
        f"cvdrift should be 0 for flat slab with betaprim=0, got {cvdrift}"
    assert np.allclose(gbdrift0, 0.0, atol=1e-10), \
