...
infile = '/Users/michaelbarnes/codes/gs2/runs/stella_tests/lres.out.nc'
...
# get theta grid
theta = np.copy(ncfile.variables['theta'][:])
...
# electrostatic potential as a function of (theta,kx,ky,t)
phi_vs_t, phi_vs_t_present \
    = read_stella_float('phi_vs_t')
# |g|^2 averaged over kx, ky, and theta
gvmus, gvmus_present \
    = read_stella_float('gvmus')
# |g|^2 averaged over kx, ky, and mu
gzvs, gzvs_present \
    = read_stella_float('gzvs')
# jacobian for transformation to (rho,alpha,theta) coordinates
jacob, jacob_present \
...
# electrostatic particle flux as function of (vpa,theta)
es_part_sym, es_part_sym_present = \
    read_stella_float('es_part_sym')
# electrostatic heat flux as function of (vpa,theta)
es_heat_sym, es_heat_sym_present = \
    read_stella_float('es_heat_sym')
# electrostatic momentum flux as function of (vpa,theta)
...
# phi(kx,ky,t) for given theta (usually outboard midplane)
...
# density fluctuation (kx,ky,t) for given theta (usually outboard midplane)
ntot_igomega_by_mode, ntot_igomega_by_mode_present = \
    read_stella_float('ntot_igomega_by_mode')
# parallel flow fluctuation (kx,ky,t) for given theta (usually outboard midplane)
upar_igomega_by_mode, upar_igomega_by_mode_present = \
    read_stella_float('upar_igomega_by_mode')
# parallel temperature fluctuation (kx,ky,t) for given theta (usually outboard midplane)
tpar_igomega_by_mode, tpar_igomega_by_mode_present = \
    read_stella_float('tpar_igomega_by_mode')
# perp termperature fluctuation (kx,ky,t) for given theta (usually outboard midplane)
