...
from stella_data import time_stella, outdir
...
for ikx in range(phi2.shape[1]-1):
    plt.semilogy(time_stella,phi2[:,ikx+1,0],'--')
    
for iky in range(phi2.shape[2]-1):
    for ikx in range(phi2.shape[1]):
        plt.semilogy(time_stella,phi2[:,ikx,iky+1])
        
        plt.xlabel('$t (v_{t}/a)$')
        plt.xlim([time_stella[0],time_stella[ntime-1]])
