...
# endif
   ...
   !> Write amplitude of electrostatic potential, apar or bpar to netCDF
...
# ifdef NETCDF
         call neasyf_write(ncid, "phi2", field2, dim_names=["t"], &
                           units="(T/q rho/L)**2", &
...
# ifdef NETCDF
         call neasyf_write(ncid, "apar2", field2, dim_names=["t"], &
                           units="(FIXME)**2", &
                           long_name="Amplitude of parallel magnetic potential A_parallel", &
                           start=[nout])
# endif
      else if (field_name == "bpar") then
# ifdef NETCDF
         call neasyf_write(ncid, "bpar2", field2, dim_names=["t"], &
                           units="(FIXME)**2", &
                           long_name="Amplitude of parallel magnetic field B_parallel", &
                           start=[nout])
# endif
      else
         call mp_abort("write_field2_nc: field_name not recognised. Aborting")
      end if
   ...
   subroutine write_field_nc(nout, field, field_name)
      ...
      if (field_name == "phi") then
# ifdef NETCDF
         call netcdf_write_complex(ncid, "phi_vs_t", field, &
...
# ifdef NETCDF
         call netcdf_write_complex(ncid, "apar_vs_t", field, &
                                   [character(len=4)::"ri", "ky", "kx", "zed", "tube", "t"], &
                                   long_name="Parallel magnetic potential A_parallel", &
                                   start=[1, 1, 1, 1, 1, nout])
# endif
      else if (field_name == "bpar") then
# ifdef NETCDF
         call netcdf_write_complex(ncid, "bpar_vs_t", field, &
                                   [character(len=4)::"ri", "ky", "kx", "zed", "tube", "t"], &
                                   long_name="Parallel magnetic field B_parallel", &
                                   start=[1, 1, 1, 1, 1, nout])
# endif
      else
...
# endif
      ...
      if (field_name == "phi") then
# ifdef NETCDF
         call neasyf_write(ncid, "phi2_vs_kxky", field2_vs_kxky, &
                           dim_names=["ky", "kx", "t "], &
                           start=[1, 1, nout], &
                           long_name="Electrostatic potential")
# endif
...
# ifdef NETCDF
         call neasyf_write(ncid, "apar2_vs_kxky", field2_vs_kxky, &
                           dim_names=["ky", "kx", "t "], &
                           start=[1, 1, nout], &
                           long_name="Parallel magnetic potential A_parallel")
# endif
...
# ifdef NETCDF
         call neasyf_write(ncid, "bpar2_vs_kxky", field2_vs_kxky, &
                           dim_names=["ky", "kx", "t "], &
                           start=[1, 1, nout], &
                           long_name="Parallel magnetic field B_parallel")
# endif
