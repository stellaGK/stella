...
! 
  integer :: bbits, b_offset, set_bits
...
contains
  ...
  subroutine init_multibox
    ...
    namelist /multibox_parameters/ bbits, b_offset
    ...
    set_bits=0
    do while (j.le.bbits)
      if(IAND(bbits,j)>0) set_bits=set_bits+1
      j=j*2
    enddo

    naky_f = 2*naky-1

    phi_buff_size = set_bits*naky_f*ntubes*(2*nzgrid+1)
    ...
    if (.not.allocated(kernels)) allocate(kernels(2*(set_bits-1),nakx))
    if (.not.allocated(conv_solv)) allocate(conv_solv(2*(set_bits-1),2*(set_bits-1)))
    if (.not.allocated(conv_idx)) allocate(conv_idx(2*(set_bits-1)))
    if (.not.allocated(rhs))  allocate(rhs(2*(set_bits-1)))
    if (.not.allocated(solution))  allocate(solution(2*(set_bits-1)))
    if (.not.allocated(bndry_ind))  allocate( bndry_ind(2*(set_bits-1)))
    if (.not.allocated(nbndry_ind)) allocate(nbndry_ind(nakx-2*(set_bits-1)))
    if (.not.allocated(akx_2)) allocate(akx_2(ikx_max))

    akx_2 = akx(1:ikx_max)
    iL = set_bits - b_offset
    iR = nakx - set_bits + 1 + b_offset
    ...
    do i = 1, nakx
      ...
      if((i==iL)            .or. & ! 0th derivative in left boundary
         ((i>set_bits).and.(i<nakx-set_bits+1)) .or. & ! physical domain
  ...
  subroutine multibox_communicate
    ...
    integer :: deriv,ibit,num,iy,iz,it,iv
