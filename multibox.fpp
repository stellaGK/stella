...
contains
  ...
  subroutine read_multibox_parameters
    ...
    use kt_grids, only: nx, nakx, copy_size, krook_size, boundary_size
  ...
  subroutine init_multibox 
    ...
    use kt_grids, only: krook_size, boundary_size
    use file_utils, only: runtype_option_switch, runtype_multibox
    use job_manage, only: njobs
...
! DSO - change communicator
  ...
  subroutine add_multibox_krook (g, rhs)
    ...
    use zf_diagnostics, only: zf_staging, calculate_zf_stress, zf_mb_krook
    ...
    do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
      do it = 1, ntubes
        do iz = -nzgrid, nzgrid
          ...
          zf_staging(:,iz,it,ivmu) = -nu_krook_mb*krook_fac(1)*g0k(1,:)
        enddo
      enddo
    enddo

    call calculate_zf_stress(zf_mb_krook)
