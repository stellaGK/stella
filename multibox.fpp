...
contains
   ...
   subroutine init_multibox
      ...
      if (.not. allocated(fsa_x) .and. (mb_zf_option_switch == mb_zf_option_no_fsa)) then
      ...
      if (job == 0 .or. job == (njobs - 1)) then
         ...
         do iv = vmu_lo%llim_proc, vmu_lo%ulim_proc
            do it = 1, vmu_lo%ntubes
               ...
               if (zonal_mode(1) .and. mb_zf_option_switch == mb_zf_option_no_fsa) then
                  do ix = 1, nakx
               ...
               do iz = -vmu_lo%nzgrid, vmu_lo%nzgrid
                  ...
                  if (zonal_mode(1)) then
                     if (mb_zf_option_switch == mb_zf_option_no_ky0) then
                        fft_kxky(1, :) = 0.0
                     elseif (mb_zf_option_switch == mb_zf_option_no_fsa) then
