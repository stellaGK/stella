module zpinch
   ...
   public :: init_local_defaults_zp
   public :: read_local_parameters_zp
   public :: communicate_parameters_multibox_zp
   public :: get_local_geo_zp
   public :: finish_local_geo_zp
   public :: local
...
contains
   ...
   subroutine init_local_defaults_zp
   ...
   end subroutine init_local_defaults_zp

   subroutine read_local_parameters_zp(nzed, nzgrid, local_out)
      ...
      namelist /zpinchgeo_parameters/ rhoc, rmaj, shift, qinp, shat, &
   ...
   end subroutine read_local_parameters_zp

   subroutine communicate_parameters_multibox_zp(surf, drl, drr)
   ...
   subroutine get_local_geo_zp(nzed, nzgrid, zed_in, zed_equal_arc, &
      ...
      ! I expect the if not being true and dpsidrho = dpsidrho * bi
...
!    call get_dpsidrho (dpsidrho)
      ...
      ! the following are both zero if the derivatives above are correct
      d2R = d2Rdr2
      d2Z = d2Zdr2
      ...
      ! I expect gbdrift = 2.0 * -dBdrho / bmag
      ...
      ! note that there's an extra factor of 1/B that's not expanded due to v_perp -> mu
      ! I expect dgbdriftdrho = 2.0 * (- d2Bdr2) / bmag
      ...
      ! I expect dcvdriftdrho = = dgbdriftdrho - gbdrift * dBdrho / bmag
      ...
      filename = "zpinch."//trim(run_name)//".input"
      ...
      close (1002)
      filename = "zpinch."//trim(run_name)//".output"
   ...
   end subroutine get_local_geo_zp
   ...
   subroutine finish_local_geo_zp

      implicit none

      call deallocate_arrays

   end subroutine finish_local_geo_zp
   ...
   subroutine get_d2dthet2(f, d2f)
      ...
      d2f(-nz) = (f(nz - 1) - 2.*f(-nz) + f(-nz + 1)) / delthet(-nz)**2
      d2f(nz) = d2f(-nz)
