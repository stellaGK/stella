...
#-------------------------------------------------------------------------------
    ...
    with xr.open_dataset(local_netcdf_file) as local_netcdf, xr.open_dataset(expected_netcdf_file) as expected_netcdf: 
        ...
        assert (not error), f'Some VMEC geometry arrays in the netcdf file did not match the previous run.' 
                
    print('  -->  All VMEC geometry data in the netcdf file matches the expected output for FFS.')
    return
