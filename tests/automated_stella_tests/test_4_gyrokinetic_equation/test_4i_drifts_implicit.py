...
# by one. Here we test the implicit drifts:
#       &physics_flags 
#         nonlinear = .true.
...
# Global variables  
input_filename = 'drifts_implicit.in'  
...
#-------------------------------------------------------------------------------
#          Check whether the explicit implicit drifts evolve correctly         #
#-------------------------------------------------------------------------------
def test_whether_drifts_implicit_evolves_correctly(tmp_path):
    ...
    with xr.open_dataset(local_netcdf_file) as local_netcdf, xr.open_dataset(expected_netcdf_file) as expected_netcdf:
        ...
        assert not np.all(local_phi2 == local_phi2[0]), f'The potential is not evolving in time, while it should.'
