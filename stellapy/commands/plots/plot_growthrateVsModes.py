#!/usr/bin/python3
''' Run plot_growthrateVsModes() as a bash command on the command prompt.'''
...
from stellapy.plot.frequencyAndGrowthrate import plot_growthrateVsModes

# If the program is run from the terminal, then collect the arguments and execute write_profiles()
def main_program():
    if __name__ == "__main__":
        function = "plot.frequencyAndGrowthrate.plot_growthrateVsModes"
        default_arguments = {\
            # Mandertory arguments for the data
            'folder' : 'MANDETORY ARGUMENT', \
            'linear_data' : 'MANDETORY ARGUMENT', \
            'input_values' : 'MANDETORY ARGUMENT', \
            # Details of the wavenumbers
            'kx' : -0.0, \
            'ki' : 'ky', \
            'k_value' : 9.0,\
            'k_delta' : 1.0, \
        ...
        plot_growthrateVsModes(**arguments)
