#!/usr/bin/python3
''' Run plot_heatfluxVsTime_linearAndLog() as a bash command on the command prompt.'''
...
from stellapy.plot.fluxes import plot_heatfluxVsTime_linearAndLog

# If the program is run from the terminal, then collect the arguments and execute write_profiles()
def main_program():
    if __name__ == "__main__":
        function = "plot.fluxes.plot_heatfluxVsTime_linearAndLog"
        default_arguments = {\
        ...
        plot_heatfluxVsTime_linearAndLog(**arguments)
