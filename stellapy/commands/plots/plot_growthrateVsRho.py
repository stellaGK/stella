#!/usr/bin/python3
''' Run plot_frequencyVsRho() as a bash command on the command prompt.'''
...
from stellapy.plot.frequencyAndGrowthrate import plot_growthrateVsRho

# If the program is run from the terminal, then collect the arguments and execute write_profiles()
def main_program():
    if __name__ == "__main__":
        function = "plot.frequencyAndGrowthrate.plot_frequencyVsRho"
        default_arguments = {\
                'folder' : 'MANDETORY ARGUMENT', \
                'k_value' : 9, \
                'k_delta' : 1, \
                'plot_reflectrometry' : True, \
                'plot_maxima' : True, \
                'SI' : False, \
                'save' : True, \
                'thesis' : True}
        arguments = get_arguments(function, default_arguments)
        plot_growthrateVsRho(**arguments)
