#!/usr/bin/python3
''' Run plot_saturatedfluxVsRho_heatfluxVsTime() as a bash command on the command prompt.'''
...
from stellapy.plot.fluxes import plot_saturatedfluxVsRho_heatfluxVsTime

# If the program is run from the terminal, then collect the arguments and execute write_profiles()
def main_program():
    if __name__ == "__main__":
        function = "plot.fluxes.plot_saturatedfluxVsRho_heatfluxVsTime"
        default_arguments = {\
                # Get the data
        ...
        plot_saturatedfluxVsRho_heatfluxVsTime(**arguments)
