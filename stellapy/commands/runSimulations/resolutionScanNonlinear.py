...
def resolutionScanNonlinear(
    ...
    for i in range(len(fprims)):
        ...
        # Scan the six paramaters
        #for parameter in ["nmu", "nvgrid","nzed", "nx", "ny", "y0"]:
        for parameter in ["nzed"]:
             
            # For each value in vec_x, create an input file and execute it
            if parameter=="nmu":    variables=vec_mu
            if parameter=="nvgrid": variables=vec_vgrid
            if parameter=="nzed":   variables=vec_nzed
            if parameter=="nx":     variables=vec_nx
            if parameter=="ny":     variables=vec_ny
            if parameter=="y0":     variables=vec_y0
            for variable in variables:
                  
                # Make a folder for this simulation
                run_directory = parameter+"scan_fprim"+str(int(fprim))+"tprim"+str(int(tiprim))+"_"+str(int(variable))
                ...
                # Add a restart folder
                restart_directory = folder / run_directory / "restart"
                os.system("mkdir "+str(restart_directory))
                 
                # Create the input file: replace the ky and the dt value
                file_name = create_inputFile(folder, run_directory, input_file, default_text, variable, fprim, tiprim, parameter)
...
def create_inputFile(folder, run_directory, input_file, default_text, variable, fprim, tiprim, parameter):
    ...
    text_before = default_text.split("species_parameters_1")[0] + "species_parameters_1"
    text_specie = default_text.split("species_parameters_1")[-1].split("/")[0] + '/' 
    text_after  = default_text.split("species_parameters_1")[-1].split("/")[1:]  
...
def main_program():
    if __name__ == "__main__":
        ...
        resolutionScanNonlinear(**arguments) 
