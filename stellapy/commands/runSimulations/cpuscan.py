    ...
    for fprim in fprims:
        for tiprim in tiprims:
            if True:
                ...
                run_directory = "CPUScan_fprim"+str(int(fprim))+"tprim"+str(int(tiprim))
                ...
                for nodes in vec_nodes:
                     
                    # Create the input file: replace the ky and the dt value
                    file_name = create_inputFile(folder, run_directory, input_file, default_text, dt, ky, fprim, tiprim, end_time, print_step, nodes)
...
def create_inputFile(folder, run_directory, input_file, default_text, dt, ky, fprim, tiprim, end_time, print_step, nodes):
    # Create the new input file
    file_name = str(input_file.stem)+"_nodes"+str(int(nodes))+".in"
