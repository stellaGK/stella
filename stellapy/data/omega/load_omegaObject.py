...
class Omega:
    ...
    def __init__(self, mode): 
        ...
        # If we have multiple (kx,ky) in the simulation, remember (ikx, iky)
        self.ikx = mode.ikx
        self.iky = mode.iky
        return
    ...
    def omega_vs_t(self):       get_omegaAndGamma(self);        return self.omega_vs_t
    @calculate_attributeWhenReadFirstTime 
    def gamma_vs_t(self):       get_omegaAndGamma(self);        return self.gamma_vs_t 
...
if __name__ == "__main__":  
    ...
    print("Test the Omega class.\n")
    for simulation in simulations: 
        for mode in simulation.modes:   
            name = "("+str(mode.kx)+", "+str(mode.ky)+")"   
            gamma = mode.omega.gamma_vs_t.gamma
            print("{:<15}".format(name), gamma[:3], gamma[-3:]) 
