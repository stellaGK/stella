...
#===============================================================================
    ...
    if get_filesInFolder(netcdf_folder, end="out.nc") or get_filesInFolder(netcdf_folder, start="restart.nc"):
        ...
        for netcdf_path in netcdf_paths:
            ...
            print(indent, '======================', '\n')
            print(indent, ' ', '{0:18}'.format("Dimensions"), '{0:18}'.format("  Shape"), '{0:35}'.format("     Value"))
            print(indent, ' ', '{0:18}'.format("----------"), '{0:18}'.format("  -----"), '{0:35}'.format("     -----"), '\n')
            for name, value in netcdf_file.dimensions.items():
                ...
                # Print the dimensions
                print( indent, '    ', '{0:18}'.format(variable+':'), '{0:18}'.format(shape), '{0:50}'.format(value))
            ...
            print(indent, '======================', '\n')
            if print_size:
                print(indent, '    ', '{0:15}'.format("Variable"), '{0:20}'.format("Shape"), '{0:20}'.format("Value"), '{0:10}'.format("      Size"), '{0:35}'.format("     Function"), '{0:15}'.format("Units"), '{0:25}'.format("Long name"))
                print(indent, '    ', '{0:15}'.format("--------"), '{0:20}'.format("-----"), '{0:20}'.format("-----"), '{0:10}'.format("      ----"), '{0:35}'.format("     --------"), '{0:15}'.format("-----"), '{0:25}'.format("---------"), '\n')
            else:
                print(indent, '    ', '{0:15}'.format("Variable"), '{0:20}'.format("Shape"), '{0:20}'.format("Value"), '{0:40}'.format("     Function"), '{0:15}'.format("Units"))#, '{0:25}'.format("Long name"))
                print(indent, '    ', '{0:15}'.format("--------"), '{0:20}'.format("-----"), '{0:20}'.format("-----"), '{0:40}'.format("     --------"), '{0:15}'.format("-----"), '\n')#, '{0:25}'.format("---------"), '\n')
            for name, value in netcdf_file.variables.items(): 
                ...
                # Print the netcdf variables
                if print_size:
                    print( indent, '    ', '{0:15}'.format(variable+':'), '{0:20}'.format(shape), '{0:20}'.format(value), '{0:10}'.format(size), '{0:35}'.format("   "+function), '{0:15}'.format(units), '{0:25}'.format(long_name))
                else:
                    print( indent, '    ', '{0:15}'.format(variable+':'), '{0:20}'.format(shape), '{0:20}'.format(value), '{0:40}'.format("   "+function), '{0:15}'.format(units))#, '{0:25}'.format(long_name[:25]))
