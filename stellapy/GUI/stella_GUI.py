...
data generated by the gyrokinetic code stella. This is the main script of stella
that can be run as an application. The GUI consists of two tabs to diagnose 
linear simulations, two tabs to diagnose nonlinear simulations, a tab to look in 
detail at the geometric configuration, a tab to diagnose the slected plasma 
profiles, and finally a tab to generate animations of the desired diagnostics. 
It is important to note that the stellapy package consists of multiple plotting
functions that can be used as stand alone scripts, as well as a gamma of commands
that can be used to check output files, run simulations and syncronize data. For
detailed information, read the user guide avaliable at github.
'''
...
# TODO: Add a movie/animation of the fluxes versus (kx,ky) 
# TODO: Remove the sign undependancy of omega and the geometry tab
...
from stellapy.GUI.interface.TabGeometry import TabGeometry
...
if __name__ == "__main__":
    ...
    root.tabProfiles         = ttk.Frame(tab_header) 
    ...
    root.tabVideos           = ttk.Frame(tab_header)    
    root.tabGeometry         = ttk.Frame(tab_header)   
    
    # Add the tabs to the tab header
    tab_header.add(root.tabSelectedFiles,    text='Simulations')
    tab_header.add(root.tabProfiles,         text='Plasma profiles')
    ...
    tab_header.add(root.tabVideos,           text='Videos')
    tab_header.add(root.tabGeometry,         text='Geometry')
    tab_header.pack(expand=1, fill='both')
    
    # Make the root accessible for the tabs
    tabs = [root.tabSelectedFiles, root.tabProfiles, root.tabConvergence, root.tabLinear, root.tabNonlinearTime, root.tabNonlinearSpatial, root.tabVideos, root.tabGeometry]
    for tab in tabs: tab.root = root 
        
    # Fill the tabs with widgets and plots.
    root.TabSelectedFiles     = TabSelectedFiles(root.tabSelectedFiles)     
    #root.tab_Profiles         = TabProfiles(root.tabProfiles)          
    root.tab_Convergence      = TabConvergence(root.tabConvergence)      
    root.tab_Linear           = TabLinear(root.tabLinear)           
    ...
    #root.tab_Videos           = TabVideos(root.tabVideos)           
    #root.tab_Geometry         = TabGeometry(root.tabGeometry)          
