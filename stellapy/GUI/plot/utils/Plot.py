        ...
        for experiment in research.plotted_experiments:
            ...
            for simulation in experiment.plotted_simulations:
                if simulation.linear:
                    simulation.plotted_modes = get_modes(simulation, self.kx_range, self.ky_range, self.modes_id)
                    research.numberOfPlottedModes += len(simulation.plotted_modes) 
    ...
    def get_title(self):  
        ...
        if self.title==None: 
            ...
            if self.ydim in standardTitles.keys(): 
                ...
                else: self.title = ""; printv("NO TITLE SET FOR: "+str(self.ydim)+", "+self.xdim) 
            else: self.title = ""; printv("NO TITLE SET FOR:"+str(self.ydim)+", "+self.xdim) 
