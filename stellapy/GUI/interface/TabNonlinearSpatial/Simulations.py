...
class Simulations: 
    ...
    def __init__(self, parent, frame):
        ...
        self.options_yquantityNames = ["Potential Squared", "Potential² (no zonal modes)", "Potential phi", "Real part of phi", "Imaginary part of phi", "Re(phi) and Im(phi)", \
            'gvmus', 'gzvs', "Heat flux", "Momentum flux", "Particle flux", "Density", "Temperature", "Parallel velocity"] 
        self.options_yquantityKeys = ["phi2", "phi2_nozonal", "phi", "phiReal", "phiImag", "gvmus", "gzvs", "phiRealImag", "qflux", "vflux", "pflux", "n", "T", "v"]
        
        # Variables for the experiment/simulation/species/yquantity
        ...
        self.var_quantity   = tk.StringVar(value=self.options_yquantityNames[0]) 
        ...
        self.lbl_quantity = ttk.Label(frame, text="Quantity: ")
        ...
        self.mnu_quantity = ttk.OptionMenu(frame, self.var_quantity, self.options_yquantityNames[0], *self.options_yquantityNames, style='option.TMenubutton')
        ...
        self.var_quantity.trace_id   = self.var_quantity.trace('w', self.change_yQuantity) 
        ...
        self.mnu_quantity["menu"].config(**self.style['menu']); self.mnu_quantity.config(width=20)  
        ...
        self.btn_timeFrame.grid( row=4, column=0, stick='NSEW', padx=(0,0), pady=(10,2), ipadx=2, ipady=1, columnspan=2)
        if True: return
...
#################################################################
    ...
    #----------------------------
    def change_yQuantity(self, calledFromGui=True, *_):
    
        # Extract what needs to be plotted on the y-axis  
        plot_id = self.options_yquantityNames.index(self.var_quantity.get())
        
        # Update the plots:
        for Plot in self.tab.Plots: 
            if Plot.initiated_canvas:
                if "Plot1DSpectrum" not in Plot.identifier:
                    Plot.Axis.set_plot(plot_id)
                    Plot.Axis.update_labelFrame(Plot.Canvas.frame)
                if "Plot1DSpectrum" in Plot.identifier:
                    Plot.Axis1.set_plot(plot_id)
                    Plot.Axis1.update_labelFrame(Plot.Canvas1.frame)
                    Plot.Axis2.set_plot(plot_id)
                    Plot.Axis2.update_labelFrame(Plot.Canvas2.frame)

        # When changing the plots, reset the axis of the graph and replot
        if calledFromGui: self.tab.reset_axes() 
        if calledFromGui: self.tab.plot()  
        return     
    ...
    #----------------------------
    def change_yQuantityWithoutPlotting(self, *_):
        self.change_yQuantity(calledFromGui=False)
        return
