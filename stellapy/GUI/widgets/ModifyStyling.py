...
# Personal modules
from stellapy.utils.config import CONFIG #@unresolvedimport
...
def _load_predefinedThemes(root):
    ...
    if os.getcwd().split(divider)[1] != 'marconi_work' and os.getcwd().split(divider)[1] != 'marconi':
        ...
        path = Path(CONFIG['CODE']['Stellapy']+'/GUI/themes/')
        path = path.as_posix()
...
def set_predefinedTheme(root, theme):
    ...
    if theme == "awdark":
        root.color = {
            ...
            'canvas': 'white'} ##33393b'} # Color of the plotting canvas, make it blend with the background
    ...
    if theme == "awlight": 
        root.color = {
            ...
            'canvas' : 'white'} #'#e8e8e7'} # Color of the plotting canvas (can't be too dark in dark theme) 
...
def load_awthemesForTkWidgets(root, theme):
    ...
    root.awthemes = { 
        'menu' : {
            ...
            "activeforeground" : root.color['fg']},\
        'toplevel' : {
            'bg': root.color['bg']},\
        'label' : {
            'bg': root.color['bg'],\
