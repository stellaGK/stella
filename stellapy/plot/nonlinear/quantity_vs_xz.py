...
def plot_quantity_vs_xz(folder, z_quantity="phi", specie=0, y=None, tstart=None, tend=None,  
    ...
    for experiment in research.experiments:
        for simulation in experiment.simulations:  
            ...
            # Plot phi(y,z) 
            subplot_quantity_vs_xz(axes[i], simulation, z_quantity, specie=specie, y=y, t_range=[tstart, tend], interpolation_step=interpolation_step,
...
#---------------------------------------- 
def subplot_quantity_vs_xz(ax, simulation, z_quantity, specie, y=None, t_range=None, interpolation_step=None,
        remove_zonal_modes=False, log=False, crange=None, ordersOfMagnitude=2):

    # Get the data
    z_vs_zx, tstart, tend, y = get_realSpaceData(simulation, z_quantity, specie, t_range, y, remove_zonal_modes) 
    specific_y = y; x, y = simulation.vec.z, simulation.vec.x
    
    # Interpolate the data
    if interpolation_step:    
        function = interp2d(x, y, z_vs_zx.T, kind='cubic')
        x = np.linspace(x[0], x[-1], int(len(x))*interpolation_step)
        y = np.linspace(y[0], y[-1], int(len(y))*interpolation_step)
        z_vs_zx = function(x,y).T              
    ...
    if log:
        z_vs_zx[z_vs_zx < 1.E-25] = np.NaN
        crange = [ np.nanmin(abs(z_vs_zx)), np.nanmax(abs(z_vs_zx)) ]  
    ...
    if not log:
        ...
        if not squared: 
            vmin = np.nanmin(z_vs_zx, axis=(0,1)); vmax = np.nanmax(z_vs_zx, axis=(0,1)) 
            vmin = np.min([vmin, -vmax]); vmax = np.max([vmax, -vmin]) 
            norm = mcolors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
        
    # Plot the surface z(x,y)  
    img = ax.pcolormesh(x, y, np.real(z_vs_zx).T, cmap=cmap, norm=norm) 
    ...
    z_label = (add_timeAndZFramesToLabel(standardLabels["normalized"][z_quantity].replace("\\hat",""), tstart, tend, specific_y)).replace("z", "y")
    ...
    ax.set_ylabel(standardLabels["normalized"]["x"])
    ax.set_xlim(xmin=np.min(x), xmax=np.max(x))
    ax.set_ylim(ymin=np.min(y), ymax=np.max(y)) 
    return ax, cbar

#-------------------------------
def get_realSpaceData(simulation, z_quantity, specie, t_range, y, remove_zonal_modes=False): 
    ...
    if y==None: 
        z_vs_zx = np.mean(z_vs_zxy, axis=2) 
    elif y=="max":  
        iy = np.argmax(np.sum(z_vs_zxy, axis=(0,1)))
        y = simulation.vec.y[iy]
        z_vs_zx = (z_vs_zxy[:,:,iy])
    elif y=="min":  
        iy = np.argmin(np.sum(z_vs_zxy, axis=(0,1)))
        y = simulation.vec.y[iy]
        z_vs_zx = (z_vs_zxy[:,:,iy])
    elif y!=None:  
        iy = np.where(simulation.vec.y>=y)[0][0] 
        y = simulation.vec.y[iy]
        z_vs_zx = (z_vs_zxy[:,:,iy])
        if y!=simulation.vec.y[iy]: print("WARNING: Choosen value of x is "+str(y)+", selected value of x is "+str(simulation.vec.y[iy])); print("vec_y = ",simulation.vec.y)
    return z_vs_zx, tstart, tend, y
...
if __name__ == "__main__" and False: 
    ...
    bash = Bash(plot_quantity_vs_xz, __doc__)    
    
    # Get the arguments and execute the script
    plot_quantity_vs_xz(**bash.get_arguments())   
...
if __name__ == "__main__":
    ...
    plot_quantity_vs_xz(folder, y=None, tstart=310)
    sys.exit()
