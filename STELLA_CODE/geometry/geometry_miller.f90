...
!===============================================================================
   ...
   contains
      ...
      subroutine deallocate_arrays
         ...
         if (allocated(b_dot_gradB)) deallocate (b_dot_gradB)
         if (allocated(b_dot_gradtheta)) deallocate (b_dot_gradtheta)
         if (allocated(b_dot_gradtheta_arc)) deallocate (b_dot_gradtheta_arc)
         ...
         ! Derivatives R(r, θ) and Z(r, θ) with respect to r and θ
         if (allocated(dR_drho)) deallocate(dR_drho)
         if (allocated(dZ_drho)) deallocate(dZ_drho)
         if (allocated(dR_dtheta)) deallocate(dR_dtheta)
         if (allocated(dZ_dtheta)) deallocate(dZ_dtheta)
         if (allocated(d2R_dr_dtheta)) deallocate(d2R_dr_dtheta)
         if (allocated(d2Z_dr_dtheta)) deallocate(d2Z_dr_dtheta)
         if (allocated(d2R_dr2)) deallocate(d2R_dr2)
         if (allocated(d2Z_dr2)) deallocate(d2Z_dr2)
         if (allocated(d2R_dtheta2)) deallocate(d2R_dtheta2)
         if (allocated(d2Z_dtheta2)) deallocate(d2Z_dtheta2)
         if (allocated(d3R_dr2_dtheta)) deallocate(d3R_dr2_dtheta)
         if (allocated(d3Z_dr2_dtheta)) deallocate(d3Z_dr2_dtheta)
         if (allocated(d2R)) deallocate(d2R)
         if (allocated(d2Z)) deallocate(d2Z)
         
         ! The r_z variable
         if (allocated(drz)) deallocate (drz)
         if (allocated(drz_dtheta)) deallocate (drz_dtheta)
      
         ! Derivatives of the magnetic field
         if (allocated(dB_drho)) deallocate(dB_drho)
         if (allocated(dB_dtheta)) deallocate(dB_dtheta)
         if (allocated(d2B_dR_dtheta)) deallocate(d2B_dR_dtheta)
         if (allocated(d2B_dr2)) deallocate(d2B_dr2)

         ! The theta and arc_length dimensions
         if (allocated(theta)) deallocate(theta)
         if (allocated(arc)) deallocate(arc)
         if (allocated(vartheta)) deallocate(vartheta)
         if (allocated(dvartheta_dr)) deallocate(dvartheta_dr)
         if (allocated(d2vartheta_dr2)) deallocate(d2vartheta_dr2)

         ! Jacobians from (r, θ, ζ) -> (R, Z, ζ)
         if (allocated(jacrho)) deallocate(jacrho)
         if (allocated(djac_drho)) deallocate(djac_drho)
         if (allocated(djacr_drho)) deallocate(djacr_drho)
         if (allocated(d2jac_dr2)) deallocate(d2jac_dr2)
      
         ! Gradients of ∇α and ∇r
         if (allocated(gradrho)) deallocate(gradrho)
         if (allocated(gradpsi)) deallocate(gradpsi)
         if (allocated(gradrho_gradtheta)) deallocate(gradrho_gradtheta)
         if (allocated(gradalpha_gradtheta)) deallocate(gradalpha_gradtheta)
         if (allocated(gradrho_gradalpha)) deallocate(gradrho_gradalpha)
         if (allocated(gradtheta2)) deallocate(gradtheta2)
         if (allocated(gradalpha2)) deallocate(gradalpha2)
         if (allocated(gradalpha_times_B_dot_gradtheta)) deallocate(gradalpha_times_B_dot_gradtheta)
         if (allocated(d2gradpsi_dr2)) deallocate(d2gradpsi_dr2)

         ! Radial variation
         if (allocated(d_bdotgradB_drho)) deallocate(d_bdotgradB_drho)
         if (allocated(d_bdotgradtheta_drho)) deallocate(d_bdotgradtheta_drho)
         ...
         if (allocated(d_Btimeskappadotgradx_drho)) deallocate(d_Btimeskappadotgradx_drho)
         if (allocated(d_Btimeskappadotgrady_drho)) deallocate(d_Btimeskappadotgrady_drho)
         if (allocated(d_BtimesgradBdotgradx_drho)) deallocate(d_BtimesgradBdotgradx_drho)
         if (allocated(d_BtimesgradBdotgrady_drho)) deallocate(d_BtimesgradBdotgrady_drho)
         if (allocated(d_nablarnablatheta_drho)) deallocate(d_nablarnablatheta_drho)
         if (allocated(d_nablatheta2_drho)) deallocate(d_nablatheta2_drho)
         if (allocated(d_nablaalphanablarho_drho)) deallocate(d_nablaalphanablarho_drho)
         if (allocated(d_nablaalphanablatheta_drho)) deallocate(d_nablaalphanablatheta_drho)
         if (allocated(d_nablaalpha2_drho)) deallocate(d_nablaalpha2_drho)
         if (allocated(d_gradalphatimesBdotgradtheta_drho)) deallocate(d_gradalphatimesBdotgradtheta_drho)
         if (allocated(d_gradpsi2_drho)) deallocate(d_gradpsi2_drho)
         if (allocated(d_gradr2_drho)) deallocate(d_gradr2_drho)
