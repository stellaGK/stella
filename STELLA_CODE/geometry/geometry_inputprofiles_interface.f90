...
!                                                                               
!###############################################################################
! TODO - Write documentation for geometry_inputprofiles_interface.f90.
!###############################################################################
module geometry_inputprofiles_interface
   ...
   public :: read_inputprof_geo
   public :: read_inputprof_spec
...
contains
   ...
   subroutine read_inputprof_geo(surf)
      ...
      ! Read in rhotor, rmin, rmaj_in, qinp and kappa
      do ir = 1, n_exp
      ...
      ! Calculate d (kappa) / drho
      call fd3pt(kappa, kapprim, dr)

      ! Calculate d (ArcSin(delta_miller)) / drho
      ...
      ! Next we need to pick out the correct flux surface and assign various local% values
      ...
      ! Safe the flux surface data to <surf>
      surf = local
   ...
   !                        Read the "input.profiles" file                      
   !****************************************************************************
   subroutine read_inputprof_spec(nspec, spec, vnew_ref)
      ...
      ! Parse in the species variables to avoid circular dependencies
      ...
      ! Reaad in the temperature and density profiles
      do ir = 1, n_exp
      ...
      ! Next we need to pick out the correct flux surface and assign various local% values
      ! Choose the first species as the reference species
      is = 1
      spec(is)%dens = 1.0
      spec(is)%temp = 1.0
     ...
     !vnewke = 3.95e-3*aref*zi**2*sqrt(0.5*mi)*loglam*ne /(sqrt(ti)*te**1.5)
