!###############################################################################
!#################### ADVANCE FIELDS WITH RADIAL VARIATION #####################
!###############################################################################
! 
...
module field_equations_radialvariation
   ...
   public :: init_field_equations_radialvariation
   public :: finish_field_equations_radialvariation
...
!###############################################################################
   ...
   subroutine calculate_phi_for_radial_variation(phi, dist, skip_fsa)
      ...
      ! Check if we need a Boltzmann response for adiabatic species
   ...
   subroutine get_radial_correction(g, phi0, dist)
      ...
      if (fphi > epsilon(0.0)) then
         ...
         do it = 1, ntubes
            do iz = -nzgrid, nzgrid
               g0k = phi1(:, :, iz, it) - denominator_fields_dr(:, :, iz) * phi0(:, :, iz, it)
               call multiply_by_rho(g0k)
               phi1(:, :, iz, it) = g0k
            end do
         end do
...
!###############################################################################
   ...
   !****************************************************************************
   subroutine init_field_equations_radialvariation
      ...
      do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
         ...
         denominator_fields_dr(iky, ikx, iz) = denominator_fields_dr(iky, ikx, iz) + tmp * wgt
      end do
      
      ! Sum the values on all processors and send them to <proc0>
      call sum_allreduce(denominator_fields_dr)
      
      ! Deallocate temporary arrays
      deallocate (g1)

      if (zonal_mode(1) .and. akx(1) < epsilon(0.) .and. has_electron_species(spec)) then
         denominator_fields_dr(1, 1, :) = 0.0
         zm = 1
      end if
      ...
      if (.not. has_electron_species(spec)) then
         ...
         denominator_fields_dr = denominator_fields_dr + efacp
      end if
      ...
      elseif (runtype_option_switch /= runtype_multibox .or. (job == 1 .and. .not. ky_solve_real)) then
         ...
         do iky = 1, naky_r
            ...
            do iz = -nzgrid, nzgrid
               ...
               if ((counter >= c_lo) .and. (counter <= c_hi)) then
                  ...
                  do ikx = 1 + zmi, nakx
                     g0k(1, :) = 0.0
                     g0k(1, ikx) = denominator_fields_dr(iky, ikx, iz)
         ...
         if (adia_elec) then
            ...
            do iz = -nzgrid, nzgrid
               ...
               do ikx = 1, nakx
                  g0k(1, :) = 0.0
                  g0k(1, ikx) = denominator_fields_dr(1, ikx, iz) - efacp
   ...
   end subroutine init_field_equations_radialvariation
   ...
   subroutine allocate_arrays_radial_variation
      ...
      use arrays, only: denominator_fields_dr
      use grids_z, only: nzgrid, ntubes
      use parallelisation_layouts, only: vmu_lo
      use grids_kxky, only: naky, nakx
      ...
      if (.not. allocated(denominator_fields_dr)) allocate (denominator_fields_dr(naky, nakx, -nzgrid:nzgrid)); denominator_fields_dr = 0.
      if (.not. allocated(denominator_fields_dr)) allocate (denominator_fields_dr(1, 1, 1)); denominator_fields_dr = 0.
   ...
   !****************************************************************************
   subroutine finish_field_equations_radialvariation
      ...
      if (allocated(denominator_fields_dr)) deallocate (denominator_fields_dr)

   end subroutine finish_field_equations_radialvariation

end module field_equations_radialvariation
