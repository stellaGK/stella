   ...
   subroutine init_neo_electrostatic_fields
      ...
      if (fphi > epsilon(0.0)) then
         ...
         do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
            ...
            wgt = spec(is)%z * spec(is)%dens_psi0 * spec(is)%zt
            call integrate_vmu(g0, iz, tmp)
...
! ================================================================================================================================================================================ !
    ...
    subroutine init_neo_electromagnetic_fields
        ...
        use neoclassical_terms_neo, only: dneo_h_dmu_global, dneo_h_dvpa_global
        ...
        if (include_apar) then 
            ...
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                ...
                g0 = spread(maxwell_vpa(:, is), 2, nmu) * maxwell_fac(is) * spread(maxwell_mu(ia, iz, :, is), 1, nvpa) * spread(vpa, 2, nmu) &
            ...
            !                                                                                                                                                          ! 
            ! denominator_fields_neo_21_gneo[iky,ikz,iz] = 
            !                                                                                                                                                          ! 
            ! ======================================================================================================================================================== !
            ...
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                ...
                * spread(aj0v(:, ikxkyz)**2 - 1.0, 1, nvpa)
                ...
                wgt = 0.5 * beta * spec(is)%z * spec(is)%dens_psi0 * spec(is)%zt * spec(is)%stm
                call integrate_vmu(g0, iz, tmp)
            ...
            ! given by:                                                                                                                                                ! 
            !                                                                                                                                                          !
            ! denominator_fields_neo_22_gneo[iky,ikz,iz] = 
            !                                                                                                                                                          !     
            ! ======================================================================================================================================================== !
        ...
        if (include_bpar) then
            ...
            !                                                                                                                                                          ! 
            ! denominator_fields_neo_13_gneo[iky,ikz,iz] = 
            !                                                                                                                                                          !     
            ! ======================================================================================================================================================== !
            
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                it = it_idx(kxkyz_lo, ikxkyz)
                ! denominator_fields13 does not depend on flux tube index, so only compute for one flux tube index.
                if (it /= 1) cycle
                iky = iky_idx(kxkyz_lo, ikxkyz)
                ikx = ikx_idx(kxkyz_lo, ikxkyz)
                iz = iz_idx(kxkyz_lo, ikxkyz)
                ...
                ! Calculate the velocity dependent factor. 
                g0 = spread(maxwell_vpa(:, is), 2, nmu) * spread(maxwell_mu(ia, iz, :, is), 1, nvpa) * maxwell_fac(is) * spread(mu, 1, nvpa) &
                * spread(aj0v(:, ikxkyz) * aj1v(:, ikxkyz), 1, nvpa)
            
                ! Multiply by the neoclassical factor.
                g0 = g0 * ( 0.5 * neo_mu_fac_global(iz, :, :, is, 1) / bmag(ia, iz) - 1.0 )

                ! Calculate denominator_fields_neo_13_gneo[iky,ikz,iz].
            ...
            !                                                                                                                                                          ! 
            ! denominator_fields_neo_23_gneo[iky,ikz,iz] = 
            !                                                                                                                                                          !     
            ! ======================================================================================================================================================== !
            ...
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                ...
                * spread(aj0v(:, ikxkyz) * aj1v(:, ikxkyz), 1, nvpa)
                ...
                ! Calculate denominator_fields_neo_23_gneo[iky,ikz,iz].
            ...
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                ...
                ! Multiply by the neoclassical factor. 
                g0 = g0 * ( spread(1.0 - aj0v(:, ikxkyz) * aj1v(:, ikxkyz), 1, nvpa) *  0.5 * neo_mu_fac_global(iz, :, :, is, 1) / bmag(ia, iz) &
                + spread(aj0v(:, ikxkyz) * aj1v(:, ikxkyz), 1, nvpa) )

                ! Calculate denominator_fields_neo_31_gneo[iky,ikz,iz].
            ...
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                ...
                ! Calculate the velocity dependent factor.
                g0 = spread(maxwell_vpa(:, is), 2, nmu) * spread(maxwell_mu(ia, iz, :, is), 1, nvpa) * maxwell_fac(is) * spread(vpa, 2, nmu) * spread(mu, 1, nvpa) &
                * spread(aj0v(:, ikxkyz) * aj1v(:, ikxkyz) - 1.0, 1, nvpa)
            ...
            do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                it = it_idx(kxkyz_lo, ikxkyz)
                ! denominator_fields33 does not depend on flux tube index, so only compute for one flux tube index.
                if (it /= 1) cycle
                iky = iky_idx(kxkyz_lo, ikxkyz)
                ikx = ikx_idx(kxkyz_lo, ikxkyz)
                iz = iz_idx(kxkyz_lo, ikxkyz)
                ...
                ! Calculate the velocity dependent factor.
                g0 = spread(maxwell_vpa(:, is), 2, nmu) * spread(maxwell_mu(ia, iz, :, is), 1, nvpa) * maxwell_fac(is) * spread(mu * mu, 1, nvpa) &
                * spread(aj1v(:, ikxkyz) * aj1v(:, ikxkyz), 1, nvpa)
            
                ! Multiply by the neoclassical factor.
                g0 = g0 * ( 1.0 - 0.5 * neo_mu_fac_global(iz, :, :, is, 1) / bmag(ia, iz) )

                ! Calculate denominator_fields_neo_33_gneo[iky,ikz,iz].
...
! =================================================================================================================================================================================== !
    ...
    subroutine allocate_neo_electromagnetic_fields
        ...
        if (include_apar) then
            ...
            if (.not. allocated(denominator_fields_neo_12_gbarneo)) &
            then; allocate (denominator_fields_neo_12_gbarneo(naky, nakx, -nzgrid:nzgrid)); denominator_fields_neo_12_gbarneo = 0. ; end if
        ...
        if (include_bpar) then
            ...
            if (.not. allocated(denominator_fields_neo_32_gbarneo)) &
            then; allocate (denominator_fields_neo_32_gbarneo(naky, nakx, -nzgrid:nzgrid)); denominator_fields_neo_32_gbarneo = 0. ; end if
...
! =================================================================================================================================================================================== !
    ...
    subroutine finish_neo_electromagnetic_fields
        ...
        if (allocated(denominator_fields_neo_12_gbarneo)) deallocate(denominator_fields_neo_12_gbarneo)
...
! =================================================================================================================================================================================== !
! ---------------------------------------------- Single Value Decomposition based Tikhonov regularization for a 2x2 real matrix inverse. -------------------------------------------- ! 
! =================================================================================================================================================================================== !
! For an ill-conditioned 2x2 field matrix, M = [[a,b],[c,d]], compute M = U * Sigma * V^T via the M^T * M eigen-decomposition.                                                        !
! Then reconstruct a regularized inverse using sigma_i/(sigma_i^2 + lam^2) for each singular value.                                                                                   !
! The inverse field matrix elements output here shoud be well-conditioned for use in the HO GK field solving. This is found to be critical for finite beta nonlinear cases.           !
! =================================================================================================================================================================================== !
  ...
  subroutine svd_2x2(a, b, c, d, U, sigma1, sigma2, V)
    ...
    ! Given the elements of the field matrix, M, compute the elements of M^T * M. 
    ! These are needed to find zeta, the rotational angle that diagonalises M^T * M.
    ! Note that MtM_21 = MtM_12 and therefore doesn't need to be explicitly computed. 
    MtM_11 = a*a + c*c
    MtM_22 = b*b + d*d
    MtM_12 = a*b + c*d
    ...
    ! Guarantee orthogonality of the second column of U.
    if (sigma2 > eps) then
        U(1,2) = MV2_1 / sigma2
        U(2,2) = MV2_2 / sigma2
       
        ! Check if columns of U are anti-aligned/reflected and fix handedness.
        if (U(1,1)*U(2,2) - U(2,1)*U(1,2) < 0.0) then
            U(1,2) = -U(1,2)
            U(2,2) = -U(2,2)
        end if
    else
  ...
  subroutine regularized_inverse_2x2(a, b, c, d, eps, Minv)
    ! Returns the Tikhonov-regularized inverse of M = [[a,b],[c,d]] in Minv.
    ! lam sets the regularization scale (same units as a,b,c,d.

    implicit none
    real, intent(in)  :: a, b, c, d, eps
