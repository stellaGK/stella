...
!###############################################################################
   ...
   subroutine calculate_fluxes_fullfluxsurface(dens, upar, pres, pflux_vs_s, &
      ...
      ! Debug
      if (debug) write (*, *) 'diagnostics::diagnostics_fluxes::calculate_fluxes_fullfluxsurface'
   ...
   subroutine get_one_flux_ffs(mom, dphidy, flxfac, flx, flx_vs_kxkyz)
      ...
      ! Multiply input moment (e.g. density, momentum, or energy) by Jacobian (called flxfac)
      ...
      do is = 1, nspec
         ...
         ! Note that the factor of 1/B that appears in the Jacobian has already 
         ! been taken into account in the numerator of the flux surface average
         do iz = -nzgrid, nzgrid
            do ikx = 1, nakx
               do iky = 1, naky
                  !if(iky == ikx == 1) then
   ...
   subroutine get_modified_fourier_coefficient(moment, moment_ky, flxfac)
      ...
      ! <is_end> is either nspec for fluxes, or ntubes when calculating |phi|^2
   ...
   !                                      Title
   !****************************************************************************
   ! TODO - Move to diagnostics_moments_fullfluxsurface.f90
   !****************************************************************************
   subroutine calculate_moments_fullfluxsurface(g, dens, upar, pres)
      ...
      use geometry, only: gradx_dot_grady, gradx_dot_gradx, gds2
      ...
      ! Species-dependent factor by which velocity moments must be multiplied to get density, pressure, etc.
      ...
      ! Iterate over the (z,mu,vpa,species) points
      do iz = -nzgrid, nzgrid
         do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
            ...
            ! Fourier tranform from ky to y space to get the kx component of <f(y,x)>_r
         ...
         ! Iterate over the (kx,ky,z) points
         do ikx = 1, ikx_max
            do iy = 1, ny
               ...
               fac1 = gradzeta_grady_R2overB2(iy, iz) * gradx_dot_grady(iy, iz) - gradzeta_gradx_R2overB2(iy, iz) * gds2(iy, iz)
               fac2 = gradzeta_grady_R2overB2(iy, iz) * gradx_dot_gradx(iy, iz) * geo_surf%shat - gradzeta_gradx_R2overB2(iy, iz) * gradx_dot_grady(iy, iz) * geo_surf%shat
   ...
   subroutine g_to_f0(g, phi, f)
      ...
      ! Iterate over the (mu,vpa,species) points
      do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
         iv = iv_idx(vmu_lo, ivmu)
         imu = imu_idx(vmu_lo, ivmu)
         is = is_idx(vmu_lo, ivmu)
