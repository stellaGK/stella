...
!!            if(proc0) write(164,*) itt, error
            ...
            do iky = 1, naky
               do it = 1, ntubes
                  do ie = 1, neigen(iky)
                     ...
                     ! phiext should contain the appropriate contribution to the time-centerd phi;
   ...
   contains
      ...
      subroutine add_streaming_contribution_phi
         ...
         use gk_parallel_streaming, only: b_dot_gradz_centerdinz, stream_sign
         use parameters_numerical, only: driftkinetic_implicit
         ...
         ! Make sure to use the correctly centerd (b . grad z) pre-factor 
         ! This will depend on the sign of vpa
         ! Note that stream_sign = -1 corresponds to positive advection velocity
         if (stream_sign(iv) > 0) then
            z_scratch = z_scratch * b_dot_gradz_centerdinz(:, -1) * code_dt * spec(is)%stm_psi0
         else
            z_scratch = z_scratch * b_dot_gradz_centerdinz(:, 1) * code_dt * spec(is)%stm_psi0
         end if
   ...
   subroutine get_contributions_from_pdf(pdf, apar, ivmu, iky, ie, rhs, source_ffs)
      ...
      ! Use the correctly centerd (b . grad z) pre-factor for this sign of vpa
      allocate (b_dot_gradz_fac(-nzgrid:nzgrid))
      if (stream_sign(iv) > 0) then
         b_dot_gradz_fac = b_dot_gradz_centerdinz(:, -1) * constant_factor
      else
         b_dot_gradz_fac = b_dot_gradz_centerdinz(:, 1) * constant_factor
      end if
