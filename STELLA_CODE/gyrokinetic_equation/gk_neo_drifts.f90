...
module gk_neo_drifts
   ...
   public :: initialised_neo_curv_drift, initialised_neo_mag_drift
   public :: init_neo_curv_drift, init_neo_mag_drift
   public :: finish_neo_curv_drift, finish_neo_mag_drift
   public :: advance_neo_curv_drift_explicit, advance_neo_mag_drift_explicit
...
! ================================================================================================================================================================================= !
    ...
    subroutine init_neo_curv_drift
        ...
        do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
            ...
            do iz = -nzgrid, nzgrid            
                neocurvx(:, iz, ivmu) = neocurvx(:, iz, ivmu) * ( vpa(iv) / ( bmag(:, iz) ** 2 ) ) * neo_fac(iz, ivmu) * B_times_kappa_dot_gradx(:, iz) &
                * maxwell_vpa(iv, is) * maxwell_mu(:, iz, imu, is) * maxwell_fac(is)
         
                neocurvy(:, iz, ivmu) = neocurvy(:, iz, ivmu) * ( vpa(iv) / ( bmag(:, iz) ** 2 ) ) * neo_fac(iz, ivmu) * B_times_kappa_dot_grady(:, iz) &
                * maxwell_vpa(iv, is) * maxwell_mu(:, iz, imu, is) * maxwell_fac(is)
            end do
        end do
...
! ================================================================================================================================================================================= !
    ...
    subroutine init_neo_mag_drift
        ...
        ! Geometry. 
        use geometry, only: bmag, B_times_gradB_dot_gradx, B_times_gradB_dot_grady
        ...
        ! Calculate neomagx and neomagy. Calculation is broken up for ease of reading.
        ! Both take the same constant factor.
        neomagx = - code_dt
        neomagy = - code_dt
        ...
        do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
            ...
            do iz = -nzgrid, nzgrid
                ! Multiply each array by its magnetic geometry factor. 
                neomagx(:, iz, ivmu) = neomagx(:, iz, ivmu) * ( mu(imu) / ( bmag(:, iz) ** 2 ) ) * B_times_gradB_dot_gradx(:, iz) &
                * ( dneo_h_dmu(iz, ivmu, 1) / ( 2 * bmag(:, iz) ) - ( neo_h(iz, ivmu, 1) - spec(is)%z * neo_phi(iz) ) ) & 
                * maxwell_vpa(iv, is) * maxwell_mu(:, iz, imu, is) * maxwell_fac(is)

                neomagy(:, iz, ivmu) = neomagy(:, iz, ivmu) * ( mu(imu) / ( bmag(:, iz) ** 2 ) ) * B_times_gradB_dot_grady(:, iz) &                 
                * ( dneo_h_dmu(iz, ivmu, 1) / ( 2 * bmag(:, iz) ) - ( neo_h(iz, ivmu, 1) - spec(is)%z * neo_phi(iz) ) ) &
                * maxwell_vpa(iv, is) * maxwell_mu(:, iz, imu, is) * maxwell_fac(is)
            end do
        end do
