...
!    nout = nout + 1
   ...
   subroutine get_fluxes_ffs(dens, upar, pres, pflx, vflx, qflx, &
      ...
      flxfac = ny * flxfac/(sum(flxfac * grho))!grad_x) )
         ...
         do iz = -nzgrid, nzgrid
            do ikx = 1, nakx
               do iky = 1, naky
                  ...
                  !   cycle 
                  !else
                  flx(is) = flx(is) - sign_torflux * 0.5 * mode_fac(iky) * flx_vs_kxkyz(iky, ikx, iz, is)
                  !end if
               end do
            end do
         end do
