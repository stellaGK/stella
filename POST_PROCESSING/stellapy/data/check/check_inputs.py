...
def check_inputs(): 
    ...
    for simulation in simulations:
        ...
        nonlinear = 'True' if inputParameters['gyrokinetic_terms']['include_nonlinear'] else 'False'
        
        # Gather the data
        indent = '    '; length = 70
        d1 = '{0:<15}'.format('nonlinear:')+'{0:<15}'.format(nonlinear)
        d2 = '{0:<15}'.format('rho:')+'{0:<15}'.format(inputParameters['geometry_vmec']['rho'])
        d3 = '{0:<15}'.format('nspec:')+'{0:<15}'.format(inputParameters['species_options']['nspec'])
        d4 = '{0:<15}'.format('tite:')+'{0:<15}'.format(inputParameters['adiabatic_electron_response']['tite'])
...
if __name__ == '__main__':
    bash = Bash(check_inputs, __doc__)
