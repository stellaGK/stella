    ...
    for input_file in input_files:
        ...
        try:
            ...
            if not os.path.isfile(fluxes_path): 
                ...
                if "qflux_vs_kxkys" not in netcdf_data.variables.keys() and "qflux_vs_kxkyzs" not in netcdf_data.variables.keys():
                    print('The fluxes(kx,ky) data was not found in the output file.')
                    continue
                if "qflux_vs_kxkyzs" not in netcdf_data.variables.keys(): z_dimension = False
                if "qflux_vs_kxkyzs" in netcdf_data.variables.keys(): z_dimension = True
            ...
            # Exit for linear simulations without a z-dimension
...
def read_fluxes3D(y_quantity, dt, netcdf_path, dl_over_B, z_dimension):
    ...
    if not z_dimension:
        ...
        vec_time = read_netcdfVariables('vec_time', netcdf_data)
        indices = get_indicesAtFixedStep(vec_time, dt)
        y_quantity = y_quantity.replace('_vs_tszkxky', '_vs_tskxky')
