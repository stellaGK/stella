...
def read_distributionFromTxtFile(path_g2_vs_z, path_g2_vs_mu, path_g2_vs_vpa, dim_species):
    ...
    if os.path.isfile(path_g2_vs_z): 
        g2_vs_zskxky = np.loadtxt(path_g2_vs_z,skiprows=1,dtype='float').reshape(-1, dim_species)[:,:,np.newaxis,np.newaxis]
        g2_vs_muskxky = np.loadtxt(path_g2_vs_mu,skiprows=1,dtype='float').reshape(-1, dim_species)[:,:,np.newaxis,np.newaxis]
        g2_vs_vpaskxky = np.loadtxt(path_g2_vs_vpa,skiprows=1,dtype='float').reshape(-1, dim_species)[:,:,np.newaxis,np.newaxis]
