module implicit_z
  ...
  public :: init_implicit_z, finish_implicit_z
...
contains
  ...
  subroutine init_implicit_z
    ...
    use run_parameters, only: wdrift_implicit, wstar_implicit
    ...
    ! We always initialise the operator inversion, rather than only doing
    ! so if stream_implicit or driftkinetic_implicit is true.
    ! TODO: No point in inverting the operator if none of the terms in z are
    ! implicit - either put that check here, or ensure that init_implicit_z
    ! isn't called.
    call init_invert_operator

  end subroutine init_implicit_z
  ...
  subroutine advance_z_implicit (g, phi, apar, bpar)
    ...
    ! TODO: Add timing for implicit in z
  ...
  end subroutine finish_implicit_z
