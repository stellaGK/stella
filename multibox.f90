...
contains
   ...
   subroutine read_multibox_parameters
      ...
      use arrays_kxky, only: nx, nakx, boundary_size, copy_size, krook_size
...
!   endif
   ...
   subroutine init_multibox
      ...
      use arrays_kxky, only: nakx, naky, akx, aky, nx, x, x_d, x0
      use arrays_kxky, only: centered_in_rho, rho_clamped, rho_d, rho_d_clamped
      use arrays_kxky, only: periodic_variation
      use arrays_kxky, only: boundary_size, krook_size
      use file_utils, only: runtype_option_switch, runtype_multibox
      use job_manage, only: njobs
   ...
   subroutine multibox_communicate(gin)
      ...
      use arrays_kxky, only: nakx, naky, naky_all, akx, aky, nx, ny, dx, dy, zonal_mode
      use arrays_kxky, only: periodic_variation, boundary_size
      use file_utils, only: runtype_option_switch, runtype_multibox
...
! DSO - change communicator
   ...
   subroutine apply_radial_boundary_conditions(gin)
      ...
      use arrays_kxky, only: naky, zonal_mode
      use arrays_kxky, only: periodic_variation, boundary_size
      use stella_layouts, only: vmu_lo
      use zgrid, only: nzgrid
   ...
   subroutine add_multibox_krook(g, rhs)
      ...
      use stella_layouts, only: vmu_lo
      use arrays_kxky, only: nakx, naky, periodic_variation, boundary_size
      use zgrid, only: nzgrid, ntubes
...
!!       It is done here because the radial grid may include an extra point
   ...
   subroutine init_mb_get_phi(has_elec, adiabatic_elec, efac, efacp)
      use arrays_kxky, only: nakx, naky, boundary_size
      use zgrid, only: nzgrid
...
!       call zgetrf(nakx,nakx,phi_solve(iky,iz)%zloc,nakx,phi_solve(iky,iz)%idx,info)
   ...
   subroutine mb_get_phi(phi, has_elec, adiabatic_elec)
      use constants, only: zi
      use arrays_kxky, only: akx, nakx, naky, zonal_mode, boundary_size
      use zgrid, only: nzgrid, ntubes
      use geometry, only: dl_over_b, d_dl_over_b_drho
      use parameters_numerical, only: ky_solve_radial
...
! domains (since we do not keep the checkboard mode)
   ...
   subroutine init_mb_transforms
      ...
      use arrays_kxky, only: nakx, naky, naky_all
