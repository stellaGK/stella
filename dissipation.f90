          ...
          do iz = -nzgrid, nzgrid
              do iv = 1, nvpa
                  do imu = 1, nmu
                      ...
                      mw(iv,imu,iz)  = maxwell_vpa(iv)*maxwell_mu(1,iz,imu)
                  end do
              end do
          end do
            ...
            do iv = 1, nvpa
                do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
                   ...
                   if (iv == 1) then
                        ...
                        mwpv   = exp(-vpap**2)*maxwell_mu(1,iz,imu)
                        ...
                        if (imu == 1) then
                            ...
                            if (mu_operator) then
                                ...
                                mwp = maxwell_vpa(iv)*exp(-2*bmag(ia,iz)*mup)
                            ...
                            if (mu_operator) then
                                ...
                                mwm = maxwell_vpa(iv)*exp(-2*bmag(ia,iz)*mum)
                        ...
                        mwmv = exp(-vpam**2)*maxwell_mu(1,iz,imu)
      ...
      do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
         ...
         do iv = 1, nvpa
            do imu = 1, nmu
                ...
                sumdelta = sumdelta + ztmax(iv,is)*maxwell_mu(1,iz,imu)*aj0v(imu,ikxkyz)*transpose(gvmutr)
            end do
        end do
...
!       g0 = 2.0*g(:,:,ikxkyz)*spread((vperp2(1,iz,:)-0.5)*aj1v(:,ikxkyz),1,nvpa)
  ...
  subroutine vpa_differential_operator_fp (h, Dh, imu, iz, is, ia)
      ...
      nuDp = spec(is)%vnew(is)*(erf(xp) - (erf(xp)-xp*(2/sqrt(pi))*exp(-xp**2)) / (2*xp**2) )/xp**3
      nupap= spec(is)%vnew(is)*2*(erf(xp)-xp*(2/sqrt(pi))*exp(-xp**2)) / (2*xp**2) / xp**3
      mwp  = exp(-vpap**2)*maxwell_mu(1,iz,imu)
      ...
      nuDm = spec(is)%vnew(is)*(erf(xm) - (erf(xm)-xm*(2/sqrt(pi))*exp(-xm**2)) / (2*xm**2) )/xm**3
      nupam= spec(is)%vnew(is)*2*(erf(xm)-xm*(2/sqrt(pi))*exp(-xm**2)) / (2*xm**2) / xm**3
      mwm  = exp(-vpam**2)*maxwell_mu(1,iz,imu)
