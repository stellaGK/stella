module fields_fluxtube
   ...
   public :: get_fields_fluxtube_vmlo, get_fields_fluxtube_kxkyzlo
...
!###############################################################################
   ...
   !============================================================================
   subroutine advance_fields_fluxtube (g, phi, apar, bpar, dist)
   ...
   subroutine get_fields_vmulo(g, phi, apar, dist, skip_fsa)
      ...
      use fields_electromagnetic, only: get_fields_em_vmulo
      ...
      if (debug) write (*, *) 'fields::get_fields_fluxtube_vmulo'
      ...
      if (fphi > epsilon(0.0) .and. .not. include_bpar) then 
         ...
         if (debug) write (*, *) 'fields::get_fields_fluxtube_vmulo::integrate_species_phi'
         call integrate_species(g_scratch, spec%z * spec%dens_psi0, phi)
         if (proc0) call time_message(.false., time_field_solve(:, 3), ' int_dv_g')
         if (debug) write (*, *) 'fields::get_fields_fluxtube_vmulo::get_phi'
         call get_phi(phi, dist, skip_fsa_local)
      end if 

      if (inlcude_apar .or. include_bpar) then 
         if (debug) write (*, *) 'fields::get_fields_fluxtube_vmulo::get_fields_em_vmulo'
         call get_fields_em_vmulo(gvmu, phi, apar, bpar, dist)
      end if 
   ...
   !============================================================================
   !> TODO-GA: remove apar from this and make it only needed for EM stella
   subroutine get_fields_fluxtube_kxkyzlo(g, phi, apar, dist, skip_fsa)
      ...
      use fields_electromagnetic, only: get_fields_em_kxkyzlo
      ...
      if (debug) write (*, *) 'fields::get_fields_fluxtube_kxkyzlo'

      ia = 1
      phi = 0.
      !! Here for now until bpar and apar are decoupled
      bpar = 0.
      apar = 0.
      ...
      if (inlcude_apar .or. include_bpar) then 
         bpar = 0.
         apar = 0.
         if (debug) write (*, *) 'fields::advance_fields_kxkyzlo::get_fields_em_kxkyzlo'
         call get_fields_em_kxkyzlo(gvmu, phi, apar, bpar, dist)
      end if 
