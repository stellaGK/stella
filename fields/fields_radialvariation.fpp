...
!###############################################################################
   subroutine get_phi_for_radial(phi, dist, skip_fsa)
      ...
      use grids_kxky, only: nakx, naky, zonal_mode
   ...
   subroutine add_adiabatic_response_radial(phi)
      ...
      use grids_kxky, only: nakx, boundary_size, rho_d_clamped
   ...
   subroutine add_radial_correction_int_species(g_in)
      ...
      use grids_kxky, only: nakx, naky, multiply_by_rho
      use run_parameters, only: ky_solve_radial
   ...
   subroutine get_radial_correction(g, phi0, dist)
      ...
      use grids_kxky, only: nakx, nx, naky, rho_d_clamped
      use grids_kxky, only: zonal_mode, multiply_by_rho
      use species, only: spec, has_electron_species
      use fields_arrays, only: phi_corr_QN, phi_corr_GA
      use fields_arrays, only: gamtot, dgamtotdr
      use fields_arrays, only: gamtot3, efac, efacp
      use dist_fn_arrays, only: kperp2, dkperp2dr
      use parameters_physics, only: adiabatic_option_switch
      use parameters_physics, only: adiabatic_option_fieldlineavg
      use stella_transforms, only: transform_kx2x_unpadded, transform_x2kx_unpadded
...
!###############################################################################
   ...
   subroutine init_fields_radial_variation
      ...
      use fields_arrays, only: efac, efacp
   ...
   subroutine allocate_arrays_radial_variation
      ...
      use fields_arrays, only: dgamtotdr
      use zgrid, only: nzgrid, ntubes
      use stella_layouts, only: vmu_lo
